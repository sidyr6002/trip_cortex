...
case "$ENV" in
  dev)
    ...
    SEARCH_AGENT_ARN="$DEV_NOVA_ACT_SEARCH_AGENT_ARN"
  ...
  staging)
    ...
    SEARCH_AGENT_ARN="$STAGING_NOVA_ACT_SEARCH_AGENT_ARN"
  ...
  prod)
    ...
    SEARCH_AGENT_ARN="$PROD_NOVA_ACT_SEARCH_AGENT_ARN"
...
sam deploy --config-env "$ENV" \
  --parameter-overrides "Environment=$ENV ClerkSecretKeyArn=$CLERK_ARN NovaActSearchAgentArn=$SEARCH_AGENT_ARN NovaActBookingAgentArn=$BOOKING_AGENT_ARN PortalCredentialsSecretArn=$PORTAL_CREDS_ARN DummyPortalUrl=$DUMMY_PORTAL_URL" \
