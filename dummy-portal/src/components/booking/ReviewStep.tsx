      ...
      <div className="p-6 sm:p-8 space-y-8 mt-2">
        ...
        <div className="grid grid-cols-2 gap-4 pt-6 border-t border-divider-light">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-content-muted mb-0.5">Route</div>
              <div className="text-sm font-bold text-content">{flight.departureAirport.code} → {flight.arrivalAirport.code}</div>
            </div>
          </div>
        </div>
      </div>
...
export default function ReviewStep({ flight, returnFlight, adults, children, pricing, onContinue }: ReviewStepProps) {
  const totalPassengers = adults + children;
  const { taxes, total } = pricing;

  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-bold text-content tracking-tight">Review Your {returnFlight ? 'Round Trip' : 'Flight'}</h2>
      ...
      {returnFlight && (
        <div className="text-sm font-semibold text-primary uppercase tracking-wider">Outbound Flight</div>
      )}
      ...
      {returnFlight && (
        <>
          <div className="text-sm font-semibold text-primary uppercase tracking-wider">Return Flight</div>
          <FlightSummaryCard flight={returnFlight} />
        </>
      )}
      ...
      <div className="bg-surface-gradient rounded-3xl p-6 sm:p-8 border border-divider-light/60 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center text-content-muted">
            <span className="font-medium">Outbound ({totalPassengers} × ${flight.pricing.pricePerPassenger})</span>
            <span className="font-semibold text-content">${(flight.pricing.pricePerPassenger * totalPassengers).toFixed(2)}</span>
          </div>
          {returnFlight && (
            <div className="flex justify-between items-center text-content-muted">
              <span className="font-medium">Return ({totalPassengers} × ${returnFlight.pricing.pricePerPassenger})</span>
              <span className="font-semibold text-content">${(returnFlight.pricing.pricePerPassenger * totalPassengers).toFixed(2)}</span>
            </div>
          )}
          <div className="flex justify-between items-center text-content-muted">
          ...
          <div className="pt-6 border-t border-divider flex justify-between items-end">
            <div>
              <div className="text-sm font-medium text-content-muted mb-1">Total Amount{returnFlight ? ' (Round Trip)' : ''}</div>
