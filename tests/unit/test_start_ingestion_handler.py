...
def _mock_service_success(mock_svc_class):
...
def test_start_ingestion_happy_path_s3_event(
    mock_svc_class, mock_aurora_class, mock_get_sfn, mock_get_bda, mock_get_config
):
    """Native S3 event parsed correctly, service called, SFN started."""
    ...
    mock_get_sfn.return_value = MagicMock()
    _mock_service_success(mock_svc_class)
    ...
    mock_get_sfn.return_value.start_execution.assert_called_once()
...
@patch("handlers.start_ingestion.AuroraClient")
@patch("handlers.start_ingestion.IngestionService")
def test_start_ingestion_happy_path_eventbridge_event(
    mock_svc_class, mock_aurora_class, mock_get_sfn, mock_get_bda, mock_get_config
):
    """EventBridge S3 event parsed correctly."""
    from handlers.start_ingestion import handler
...
def test_start_ingestion_non_pdf_skipped(mock_aurora_class, mock_get_config):
    ...
    event = {"detail": {"bucket": {"name": "bucket"}, "object": {"key": "uploads/policy.txt"}}}
