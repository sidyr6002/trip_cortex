...
def test_returns_valid_reasoning_result():
    result = apply_graceful_degradation("b-1", "e-1", "HYD", "ORD", FUTURE_STR)
...
def test_return_date_parsed_when_provided():
    return_str = (FUTURE + timedelta(days=7)).isoformat()
    result = apply_graceful_degradation("b-1", "e-1", "HYD", "ORD", FUTURE_STR, return_date=return_str)
    assert result.plan.parameters.return_date is not None


def test_return_date_none_when_omitted():
    result = apply_graceful_degradation("b-1", "e-1", "HYD", "ORD", FUTURE_STR)
    assert result.plan.parameters.return_date is None
...
def test_output_matches_reasoning_result_schema():
    ...
    # Must be re-parseable by ReasoningResult
...
def test_handler_output_consumable_by_validate_plan(mock_config, mock_dynamo, mock_audit):
    ...
    # ValidatePlan handler calls ReasoningResult.model_validate on the output
...
def test_handler_tolerates_missing_error_fields(mock_config, mock_dynamo, mock_audit):
    ...
    output = handler(event, None)  # must not raise
